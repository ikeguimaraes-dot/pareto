...
export async function proxy(request: NextRequest) {
  ...
  if (pathname === '/' || pathname === '/login' || pathname === '/auth/callback') {
    if (user) {
      ...
      const dest = profile?.role === 'admin' ? '/admin' : '/registro'
  ...
  // Verificação de papel para rotas /admin
...
// Alias para compatibilidade enquanto a API ainda aceita as duas formas
